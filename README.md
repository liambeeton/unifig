...
`docs/COMPATIBILITY.md` is the answer, and it is generated rather than written: CI runs the whole test suite against every UniFi Network version in `compatibility.yaml`, and the table is built from what those runs did. It also says which areas a container cannot answer for at all, which Controller version the recorded responses behind them came from, and — where a test rests on something measured in a live write session on other firmware, which a read-only recording could never hold — the version that session was on (`docs/adr/0036-a-test-names-the-controller-its-evidence-was-measured-on.md`). The versions are deliberately not repeated here — the table is the one place they live, and a list in this file is a list that goes stale.
...
If you already made that policy in the UniFi UI — same name, same pair — unifig works with it rather than refusing the site. The Controller keeps both and evaluates yours first, and yours is the only one of the two with an id to write to, so yours is the one your config entry means; the generated one is shadowed. It is still left out of the file, still counted in the notice, and `--prune` still will not delete it. What is still refused is two policies *of yours* sharing a name and a pair, because nothing has answered which one your file meant.

The return rule the Controller generates beside each of your own allow policies is left out too, for its own reason: your file already states it, as the verdict of the policy it belongs to, so a `"<name> (Return)"` entry beside it would be a second line about the same thing — and one your next plan would try to create. Those are not in the count, because the count is what the file leaves unexplained and that one your file explains. The `Allow Return Traffic` policies a migrated router ships *are* in it: the policies they answer for are the Controller's own, and went out with the rest. `--prune` will not delete a return rule either, and that is the same sentence rather than a second promise: unifig never creates, names or deletes one, so a policy your file leaves out for that reason is not one prune may act on.
