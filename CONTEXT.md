...
**Return Rule**:
The second policy the Controller generates for the reply traffic of a Firewall Policy created allowing, named `<name> (Return)` after its parent and reclaimed with it. The Controller generates it below where it puts a policy unifig created, which is the wrong way round and is why a blocking policy on its pair is reordered past it (ADR-0033). Also its **companion**, which is the word for its relationship to that parent and the one a plan uses when the parent is what the operator is looking at. Not a Resource: unifig never creates, names or deletes one directly. What it owns is the **request** — a field on the policy, which the Controller refuses beside any verdict that closes a path (ADR-0022) and acts on in both directions on an update as well as a create (ADR-0026). It refuses the same request for a policy whose destination is the External Zone, whatever the verdict: an allow out to the internet is taken without complaint and simply generates nothing, so unifig does not ask, and a plan about one promises no companion (ADR-0030). Both conditions are one predicate the create, the update and the plan share, because a second condition arriving is exactly what would have left three copies of the first disagreeing. So a companion follows the config rather than the policy's history, and a plan states its arrival or departure as a field of the change rather than announcing it as a consequence. The request and the companion are asked separately and either disagreeing with the verdict is a change, because a policy whose request is already right can still be missing the companion it asked for — a state the plan was silent about, and silence is what stopped it from ever being repaired. The companion half is asked only of a policy whose companion would carry that name, which is not a Generated Policy: the Controller names its own `Allow Return Traffic` on the reverse pair, so the absence of a `<name> (Return)` beside one says nothing (ADR-0034). Export never writes one, and not for the reason it leaves a Generated Policy out: an entry of its own would be a second, competing statement about an object the config already states as the verdict of its parent. Which is why the exclusion reads `connection_state_type` rather than the `_id` shape, though every companion measured carries the composite of a Generated Policy too. Prune never deletes one either, on that same predicate: what disqualifies a companion from the file disqualifies it from a deletion, and two halves of that guarantee reading different fields are two halves that can drift (ADR-0028).
