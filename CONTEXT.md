...
**Generated Policy**:
A Firewall Policy the Controller computes for a pair of Zones rather than storing — the eighty-six a migrated router ships, and every Return Rule. It is readable and matchable like any other, and it is the one thing in the model that can never be written: its `_id` is not a handle but a description of where it came from, the two Zone ids and the index run together, and the Controller answers 404 `api.err.FirewallPolicyNotFound` to it on GET and on PUT alike (ADR-0027). The id an object the Controller really stores is addressed by — twenty-four hex characters, handed out on create — is its **document handle**, and having one is exactly what a Generated Policy lacks. So a change to one is a change no plan may promise — it is held back and said out loud as a Caveat, and the way to overrule one is a policy of the operator's own on the same pair, which takes precedence because the Controller's sits at the lowest there is. What makes a Policy generated is the shape of its `_id` rather than its `predefined` marker; the marker says whose object it is, which is what Prune asks and a different question.
