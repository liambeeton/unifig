...
**Return Rule**:
The second policy the Controller generates for the reply traffic of a Firewall Policy created allowing, named `<name> (Return)` after its parent and reclaimed with it. Also its **companion**, which is the word for its relationship to that parent and the one a plan uses when the parent is what the operator is looking at. Not a Resource: unifig never creates, names or deletes one — asking for it is a field on the create, which the Controller refuses beside any other verdict (ADR-0022) — so a plan announces one rather than planning it. Which policies have one is a fact about their history rather than about the config, and a plan about an update says which history the policy in front of the operator is on (ADR-0025).
