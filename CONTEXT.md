...
**Export**:
Generating YAML config from the live Controller state. The brownfield adoption path. What it writes is scoped to what a Plan could act on rather than to what it can word — a Generated Policy words perfectly well and is left out anyway, because an entry naming one could only ever produce a Caveat (ADR-0028). Everything it leaves out it says on stderr, so a file that came back short says why, and stdout carries nothing but YAML.
