...
- The two halves of this rule meet where prune is concerned, and the meeting was a bug. A file with `networks:` and no `wlans:` key puts a network at stake and no WLAN, so prune proposed deleting a network that a WLAN it may not touch was still on — and the Controller refuses that. So the referencing section is now read even where the file leaves it out, and the deletion is not proposed at all (ADR-0014, issue #22). That is the bullet above arriving at its second reason rather than an exception to this one: what the file's sections decide is still only what unifig will change.
