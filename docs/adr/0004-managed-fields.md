...
- "Puts the whole object back" is a claim about the request, and a struct is not the object. Where the live Resource is read through `go-unifi`, whatever that library does not model — and whatever `omitempty` elides at its zero value — is missing from the body, so under a replacing endpoint the update deletes it. Measured on the v2 firewall-policy endpoint and fixed there by merging into the JSON the Controller sent (ADR-0021); the v1 verbs have the same shape and nobody has measured them.
